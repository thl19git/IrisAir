...
app.get('/startSession', (req, res) => {
  ...
  db.get('SELECT device FROM users WHERE email = ?', [req.user], (err, row) => {
    if(err || row.device == null){
      ...
      fetch(apiAddr+"/session/start?encrypted_serial_number="+encryptCode(row.device) , {
        method: "POST"
      })
        .then( () => {
      ...
      fetch(apiAddr+"/session/feeling", {
        ...
        .then( () => {
          fetch(apiAddr+"/session/description", {
            ...
            .then( () => {
              fetch(apiAddr+"/session/stop?encrypted_serial_number="+encryptCode(row.device), {
                method: "POST",
              })
                .then( () => {
...
app.get('/getAlerts', (req, res) => {
  ...
  db.get('SELECT device, alert_id, latest_score, session FROM users WHERE email = ?', [req.user], (err, row) => {
    if(err || !row || row.device == null || row.session == 0){
      res.json({alert: false, message: null});
    } else {
      fetch(apiAddr+"/predict?encrypted_serial_number="+encryptCode(row.device),{method: "POST"})
      .then( (response) => {
        response.json()
        .then((data) => {
          var feeling = data[0];
          ...
          if((row.alert_id == 0 || feeling != row.latest_score)&&feeling<6&&feeling>0&&(Math.abs(tempDiff)>0.5||Math.abs(humidityDiff)>0.5)){
...
app.get('/getLatestData', (req, res) => {
  ...
  db.get('SELECT device, session FROM users WHERE email = ?', [req.user], (err, row) => {
    if(err || !row || row.device == null || row.session == 0 ){
      ...
      fetch(apiAddr+"/session/data?encrypted_serial_number="+encryptCode(row.device),{method: "POST"})
      .then( (response) => {
        response.json()
        .then((data) => {
          if(data.length<1){
...
app.get('/sessionData', (req, res) => {
  ...
  db.get('SELECT device, session FROM users WHERE email = ?', [req.user], (err, row) => {
    if(err || row.device == null){
      ...
      fetch(apiAddr+"/session/extract?encrypted_serial_number="+encryptCode(row.device),{method: "POST"})
      .then( (response) => {
        response.json()
        .then((data) => {
          if(data.length < 1){
            ...
            minscore = 10;
            maxscore = 0;
            var num_sessions = data.length - row.session;
